...
if __name__ == "__main__":
	...
	diff = min(predAmpCoalesce)-max(predAmp)
	diffN = max(predAmpNCoalesce)-min(predAmpN)
	tDiff = min(x1)-max(cleanTime)
	tDiffN = min(x2)-max(cleanTimeN)
	error = predAmp[-1]-predAmp[-2]
	errorN = predAmpN[-1]-predAmpN[-2]

	if abs(cleanTime[0]) > abs(cleanTimeN[0]):
		time = [t+tDiffN for t in time]
		cleanTime = [t+tDiffN for t in cleanTime]
		cleanTimeN = [t+tDiffN for t in cleanTimeN]
	else:
		time = [t+tDiff for t in time]
		cleanTime = [t+tDiff for t in cleanTime]
		cleanTimeN = [t+tDiff for t in cleanTimeN]
	predAmpCoalesce = [t-diff+error for t in predAmpCoalesce]
	predAmpNCoalesce = [t-diffN for t in predAmpNCoalesce]
